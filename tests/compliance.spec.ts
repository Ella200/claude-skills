/**
 * LuraBio — Research Use Only Compliance Gate Tests
 *
...
async function fillBillingFields( page: Page ): Promise<void> {
    ...
    await page.fill( '#billing_email',      'researcher@lurabio.com' );
}
...
test.describe( 'Research Use Only Compliance Gate', () => {
    ...
    test( 'compliance checkbox is visible on checkout page', async ( { page } ) => {
        await expect( page.locator( '#lurabio_research_use_only' ) ).toBeVisible();
    } );
    ...
    test( 'compliance checkbox is positioned above the Place Order button in the DOM', async ( { page } ) => {
        const checkboxBox = await page.locator( '#lurabio_research_use_only' ).boundingBox();
    ...
    test( 'compliance checkbox has an associated label element (accessibility)', async ( { page } ) => {
        const id    = await page.locator( '#lurabio_research_use_only' ).getAttribute( 'id' );
    ...
    test( 'blocks order placement when checkbox is unchecked', async ( { page } ) => {
        await expect( page.locator( '#lurabio_research_use_only' ) ).not.toBeChecked();
    ...
    test( 'does not surface compliance error when checkbox is checked before submit', async ( { page } ) => {
        await page.check( '#lurabio_research_use_only' );
        await expect( page.locator( '#lurabio_research_use_only' ) ).toBeChecked();
    ...
    test( 'gate blocks order even when JS adds lurabio-field-error class (server is authoritative)', async ( { page } ) => {
        // Simulate JS having run and applied error class — server must still block
        await page.evaluate( () => {
            const field = document.querySelector( '.lurabio-compliance-field' );
            if ( field ) field.classList.add( 'lurabio-field-error' );
        } );
